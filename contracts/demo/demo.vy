...
@external
def run_demo2(_demo2: address):
    Demo(_demo2).compute_add_mul(2, 5)
    self.g = msg.gas

addr: public(address)

@external
def create_2(_target: address):
    salt: bytes32 = keccak256("1000")
    self.addr = create_minimal_proxy_to(_target, salt=salt)