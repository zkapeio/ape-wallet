...
def test_transfer_erc20(bob, w3, proxy, base, token, nft, base_sign_message, new_account, get_key):
    ...
    # encode token bound accounts data
    data = proxy.token_bound_accounts.encode_input(new_account, nft, 1)

    owner = bob.address
    account = new_account_address
    transaction_to = proxy.address
    ...
    a, b, c = account_contract.token()
    assert b == nft.address and c == 1
